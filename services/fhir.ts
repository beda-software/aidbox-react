...
export async function deleteFHIRResource<R extends AidboxResource>(
  ...
  if (!inactiveMappingItem) {
    console.error(
      `Please specify inactiveMapping for ${resource.resourceType} to mark item deleted`
    );
...
export function extractBundleResources<T extends AidboxResource>(
  ...
  const entriesByResourceType = _.groupBy(
    bundle.entry,
    entry => entry.resource!.resourceType
  );

  return _.mapValues(entriesByResourceType, entries =>
    _.map(entries, entry => entry.resource!)
  );
}
...
export function getIncludedResource<T extends AidboxResource>(
  ...
  return _.find<T>(
    resources[reference.resourceType],
