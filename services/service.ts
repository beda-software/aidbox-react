...
export async function resolveServiceMap<I, F>(
    ...
    const keys = _keys(promisesMap);
    const values = _values(promisesMap);
    ...
    if (isSuccessAll(responses)) {
        return success(
            _zipObject(
                keys,
                _map(responses, (response) => response.data)
            ) as I
        );
    } else {
        return failure(_compact(_map(responses, (response) => (isFailure(response) ? response.error : undefined))));
    }
}
